...
export default function ResetClient() {
  ...
  // Ensure the session is present (the email link should have gone through /auth/callback)
  useEffect(() => {
    ;(async () => {
      try {
        ...
        if (!data.session) setMsg('Reset link expired or invalid. Request a new one.')
      } catch (e: any) {
      ...
      <div className="space-y-3">
        ...
        {!sessionReady && (
          <p className="mt-2 text-sm">
            <a href="/auth/forgot" className="underline">Request a new reset link</a>
          </p>
        )}
      </div>
    </main>

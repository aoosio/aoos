...
export async function POST(req: Request) {
  try {
    ...
    if (body.org && typeof body.org === 'object') {
      ...
      const it = src.industry_type ?? src.org_type ?? src.type
      if (it !== undefined) {
        if (shape.orgs.cols.industry_type) upd.industry_type = it
        else if (shape.orgs.cols.org_type) upd.org_type = it
        else if (shape.orgs.cols.type) upd.type = it
      }
    ...
    if (body.whatsapp && typeof body.whatsapp === 'object') {
      ...
      if (shape.waChannel.cols.phone_number_id && src.phone_number_id !== undefined) row.phone_number_id = String(src.phone_number_id).trim()
      if (shape.waChannel.cols.waba_id && src.waba_id !== undefined) row.waba_id = String(src.waba_id).trim()

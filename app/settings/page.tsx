  ...
  useEffect(() => {
    ;(async () => {
      ...
      if (o) {
        ...
        const { data: ch } = await supabase.from('channel_whatsapp').select('phone_number_id,waba_id').eq('org_id', o.id).maybeSingle()
        setCw({
    ...
    <div className="grid gap-6">
      ...
      <Card>
        ...
        <div className="mt-3 grid gap-3 sm:grid-cols-3">
          ...
          <div>
            <Label>WABA ID</Label>
            <Input value={cw.waba_id} onChange={(e) => setCw({ ...cw, waba_id: e.target.value })} placeholder="e.g., 987654321" />
          </div>
          <div>
            <Label>Access Token (paste once)</Label>
            <Input type="password" value={token} onChange={(e) => setToken(e.target.value)} placeholder="Meta token" />
          </div>
